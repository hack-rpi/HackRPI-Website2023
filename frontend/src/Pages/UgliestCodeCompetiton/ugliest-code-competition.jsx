...
const UgliestCodeCompetition = () => {
  ...
  const toggleWindow = (index) => {
    setShowWindows((prevShowWindows) => {
      const newShowWindows = Array(ugliestCodeArray.length).fill(false);
      newShowWindows[index] = !prevShowWindows[index];
  ...
  return (
    <section id="ugliest-code-competition" style={sectionStyle}>
      ...
      {showWindows.map((showWindow, index) => (
        showWindow && (
          <CodePopup
            key={index} // Use a unique key based on your data structure
            title={ugliestCodeArray[index].title}
            onClose={() => toggleWindow(index)}
            code={ugliestCodeArray[index].code}
          />
        )
      ))}
    </section>
